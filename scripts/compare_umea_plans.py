...
mlc_scatter_range_mms = [80, 160, 240]
field_sizes = [100] # [50, 100, 150, 200, 400]

for mlc_scatter_amplitude in mlc_scatter_amplitudes:
    for mlc_scatter_range_mm in mlc_scatter_range_mms:
        for field_size in field_sizes:
            machine_config = MachineConfig(preset="src/pydose_rt/data/machine_presets/umea_10MV.json", ct_array_shape=(500, 500, 500), resolution=(1.0, 1.0, 1.0), number_of_leaf_pairs=60, tpr_20_10=0.742, mlc_scatter_amplitude=mlc_scatter_amplitude, mlc_scatter_range_mm=mlc_scatter_range_mm)
            ...
            print(f"Scatter amplitude: {mlc_scatter_amplitude}\tScatter range: {mlc_scatter_range_mm}\tFS: {field_size}\tResults: {np.mean(results)}")
            if do_plot:
