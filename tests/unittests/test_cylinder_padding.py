...
def test_engine_warns_about_what_rotation_would_clip(default_machine_config, default_device):
    """Body wider than the rotation cylinder: the engine says how much it loses."""
    from pydosert import DoseEngine
    from pydosert.data import BeamSequence

    shape, res = (8, 40, 40), (2.0, 2.0, 2.0)
    iso = (8.0, 40.0, 40.0)
    density = torch.zeros(1, *shape, device=default_device)
    density[:, :, 2:-2, 2:-2] = 1.0                 # corners stick out of the inscribed circle
    seq = BeamSequence.create([0.0], default_machine_config.number_of_leaf_pairs, (400, 400), iso,
                              device=default_device, dtype=torch.float32, requires_grad=False)
    engine = DoseEngine(machine_config=default_machine_config, kernel_size=5,
                        dose_grid_spacing=res, dose_grid_shape=shape, beam_template=seq,
