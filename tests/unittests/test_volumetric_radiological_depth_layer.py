...
def test_uniform_water_linear_growth_along_depth(layer_factory, default_ct_array_shape,
                                                 default_resolution, default_device, default_dtype):
    ...
    assert torch.allclose(column, expected, atol=1e-3), \
...
def test_rotation_changes_depth_profile(layer_factory, default_ct_array_shape,
    ...
    # Slab of density in the first half along the CT depth direction.
    ct = torch.zeros((1, H, D, W), device=default_device, dtype=default_dtype)
    ct[:, :, : D // 2, :] = 1.0
    ...
    # At gantry=90 the CT depth slab becomes a lateral slab in BEV; the column
    # at a position that does NOT cross the slab laterally sees no density.
