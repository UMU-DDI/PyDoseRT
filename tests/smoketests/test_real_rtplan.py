...
def test_real_rtplan(rtp_data_dir, rtp_dose_path, rtp_plan_path):
    ...
    dose_layer_1 = DoseEngine(config_1, 55)
    dose_1 = dose_layer_1(torch.tensor(np.array(leafs_1), dtype=torch.float32, device=config_1.device), torch.tensor(np.array(mus_1), dtype=torch.float32, device=config_1.device), ct_image=torch.tensor(ct_slices, dtype=torch.float32, device=config_1.device), permute_ct=False)
    ...
    dose_layer_2 = DoseEngine(config_2, 55)
    dose_2 = dose_layer_2(torch.tensor(np.array(leafs_2), dtype=torch.float32, device=config_2.device), torch.tensor(np.array(mus_2), dtype=torch.float32, device=config_2.device), ct_image=torch.tensor(ct_slices, dtype=torch.float32, device=config_2.device), permute_ct=False)
    doses = [dose_1, dose_2]
