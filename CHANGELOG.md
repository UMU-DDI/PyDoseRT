...
- `MachineConfig.electron_contamination` (`[amplitude, sigma_mm, range_mm]`): a contamination-electron term the photon engine adds to the dose, the fluence blurred by a Gaussian and falling off as `exp(-(d/range)^2)` with radiological depth. The pencil kernel is photons only and under-predicts the build-up by an amount that grows with the field size -- against the Umea 10 MV depth doses, 42% against a measured 57% at 1 mm in a 10x10 field, and 42% against 73% in a 20x20. It is None on every preset, so dose is unchanged unless a machine sets it.
...
### Changed
- **Breaking**: `PencilBeamModel.get_pencil_beam` no longer zeroes kernels below 0.5 mm radiological depth; `depth_threshold_mm` defaults to 0. The cutoff zeroed a whole depth plane, so under oblique skin it also zeroed tissue upstream of where the central axis enters. Pass `depth_threshold_mm=0.5` for the old behaviour.
