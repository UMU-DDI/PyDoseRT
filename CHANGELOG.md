...
- `pad_to_cylinder`, `crop_from_cylinder` and `body_cylinder_radius_mm` in `pydosert.data.loaders`. The engines rotate each axial slice about the isocentre, so only the inscribed circle of the axial plane survives every gantry angle and tissue further out is silently treated as air. `pad_to_cylinder` pads the axial plane symmetrically so the isocentre is centred and the patient's body radius (`body_cylinder_radius_mm`) fits; `crop_from_cylinder` maps computed dose back to the original grid.
